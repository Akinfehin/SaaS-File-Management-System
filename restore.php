...
<?php
	...
	if(ISSET($_REQUEST['ID'])){
		...
		$query=$conn->query("SELECT * FROM `trash` WHERE `ID` = '$ID'") or die(mysqli_error());
		$fetch=$query->fetch_array();
         ...
         rename("../uploads1/backup_files/".htmlentities($fetch["NAME"]), $folder);
     
         $conn->query("INSERT INTO `upload_files`(ID, NAME, SIZE, DOWNLOAD, TIMERS, ADMIN_STATUS, EMAIL, FOLDERSELECT, VARIABLE, `TYPE`, LOGIN_ID) VALUES('$id', '$name', '$size', '$download', '$time', '$status', '$uploads', '$targetFile', '$foldername','$type','$login')") or die(mysqli_error($conn));

		$conn->query("DELETE FROM `trash` WHERE `ID` = '$ID'") or die(mysqli_error($conn));
