<?php
	...
	if(ISSET($_REQUEST['ID'])){
		...
		$query = $conn->query("SELECT * FROM `upload_adminfiles2` WHERE `ID` = '$ID'") or die(mysqli_error());
		$fetch = $query->fetch_array();
         ...
         $getID =  htmlentities($fetch['GET_ID']);      
          ...
          $destination = "../uploads/backup_filesadmin2/".htmlentities($fetch["NAME"]);
          rename($folder, $destination);


	    $conn->query("INSERT INTO `trash_admin2` VALUES('$id', '$name', '$size', '$download', '$time', '$status', '$uploads', '$targetFile', '$foldername','$type', '$login','$getID')") or die(mysqli_error());
		$conn->query("DELETE FROM `upload_adminfiles2` WHERE `ID` = '$ID'") or die(mysqli_error());
		echo "<script type='text/javascript'>alert('File Has been Deleted!');document.location='view_archiveadmin2.php'</script>";
		}
