<?php
	...
	if(ISSET($_REQUEST['ID'])){
		...
		$query = $conn->query("SELECT * FROM `upload_adminfiles1` WHERE `ID` = '$ID'") or die(mysqli_error());
	    ...
	    $conn->query("INSERT INTO `trash_admin1` VALUES('$id', '$name', '$size', '$download', '$time', '$status', '$uploads', '$targetFile', '$foldername','$type', '$login','$getID')") or die(mysqli_error());
		$conn->query("DELETE FROM `upload_adminfiles1` WHERE `ID` = '$ID'") or die(mysqli_error());
		echo "<script type='text/javascript'>alert('File Has been Deleted!');document.location='view_archiveadmin1.php'</script>";
		}
