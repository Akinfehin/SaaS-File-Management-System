 ...
 <div id="loader"></div>
  <main class="pt-5 mx-lg-5">
    ...
    <a href="add_document.php"><button type="button" class="btn btn-info"><i class="fas fa-chevron-circle-left"></i>  Document</button></a>
     ...
     <a href="view_archiveadmin2.php"><img src="_Assets/img/user-trash-full-128.png" class="imgs" width="40px" height="40px" title="View Archive" style="float: right;"></a>
    </div>
   <hr>
 <div class="col-md-12">
 <table id="dtablex" class = "table table-striped">
  <thead>
    <th>Filename</th>
    <th>FileSize</th>
    <th>Uploader</th>  
...
<?php 
        error_reporting(0);
         require_once("_Assets/include/connection.php");
          ...
          $query = $conn->query("SELECT *,b.ID as IDS  FROM upload_adminfiles1 a INNER JOIN upload_adminfiles2 b ON a.ID = b.GET_ID WHERE b.GET_ID = '$getID'") or die (mysqli_error($conn));
        if($query->num_rows === 0) echo'<div class="alert alert-danger" role="alert">No Record Found!!!</div>';
          while($file=$query->fetch_array()){
             ...
             $loginid =  htmlentities($file['LOGIN_ID']);
                ...
                $var =  '<a href="subfolder_superadmin1.php?SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c='.$idx.'""><img src="_Assets/img/floder-close-128.png" width="30px" height="25px" title="View File">&nbsp;'.substr($foldername,11).'</a> &nbsp;<i class="fas fa-file-medical sub_click" style="color:#007bff" title="Add new file" data-id2="'.$idx.'"></i>';
               }
             if($type == "sub_file2"){
                        $var3 =  '<a href="download.php?f='.$foldername.''.$name.'"><img src="_Assets/img/search-download-128.png" width="30px" height="30px" title="View And Download File"></a> | <a href="remove_adminfile2.php?ID='.$id.'"><img src="_Assets/img/-_Trash-Can-Delete-Remove--128.png" width="30px" height="30px" title="Remove File"></a>';
                     }else{
                        $var3 =  '';
                    }
