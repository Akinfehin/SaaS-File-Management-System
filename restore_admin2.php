...
<?php
	...
	if(ISSET($_REQUEST['ID'])){
		...
		$query=$conn->query("SELECT * FROM `trash_admin2` WHERE `ID` = '$ID'") or die(mysqli_error());
		$fetch=$query->fetch_array();
         ...
         rename("../uploads/backup_filesadmin2/".htmlentities($fetch["NAME"]), $folder);
     
         $conn->query("INSERT INTO `upload_adminfiles2`(ID, NAME, SIZE, DOWNLOAD, TIMERS, ADMIN_STATUS, EMAIL, FOLDERSELECT, VARIABLE, `TYPE`, LOGIN_ID, GET_ID) VALUES('$id', '$name', '$size', '$download', '$time', '$status', '$uploads', '$targetFile', '$foldername','$type','$login','$getID')") or die(mysqli_error($conn));

		$conn->query("DELETE FROM `trash_admin2` WHERE `ID` = '$ID'") or die(mysqli_error($conn));
