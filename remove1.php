<?php
	...
	if(ISSET($_REQUEST['ID'])){
		...
		$query = $conn->query("SELECT * FROM `upload_files1` WHERE `ID` = '$ID'") or die(mysqli_error());
		...
		$conn->query("DELETE FROM `upload_files1` WHERE `ID` = '$ID'") or die(mysqli_error());
		echo "<script type='text/javascript'>alert('File Has been Deleted!');document.location='view_archive1.php'</script>";
		}
